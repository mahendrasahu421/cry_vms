            ...
            if (($this->session->userdata('emp_id') != "" || $this->session->userdata('emp_id') != null)) {
                ...
                if ($role == 1) {
                    ...
                    if ($this->input->post('start_new') != "" && $this->input->post('end_new') != "" &&  $this->input->post('state_name') != "" && $this->input->post('region_id') != "") {
                        ...
                        $searchArray = array("Mr. Deepanshu Mittal", "Hindi College, Delhi", "Bitapi Baruah", "Senior Manager", "Deepanshu", "Data Analysis and documentation");
                        $latsarrayTemplate = array(
                            ...
                            $data['feedbackCertifecate'][0]['first_name']." ".$data['feedbackCertifecate'][0]['last_name'],
                            rtrim($skils, ", "),
                        );
                    ...
                    if ($this->input->post('start_new') != "" && $this->input->post('end_new') != "" &&  $this->input->post('state_name') != "" && $this->input->post('region_id') != "") {
                        ...
                        $where = "fd.creation_date>='" . $date_from . "' and fd.creation_date<='" . $date_to . "' and i.state_id=" . $state_name . "  AND i.status =8 AND isr.status = 2 AND fd.status=1";
                        $data['feedbackCertifecate'] = $this->Admin_model->send_certificate_by_feedback($where, $empId, $role);
                        $skillId = $data['feedbackCertifecate'][0]['task_keyword'];
                        $skill_name = explode(",", $skillId);
                        $skils = "";
                        for ($i = 0; $i < sizeof($skill_name); $i++) {
                            $assig_name = $this->Crud_modal->fetch_single_data("skill_name", "skills", "skill_id= '" . $skill_name[$i] . "'");
