<style>
	table td {
		white-space: initial !important;
	}

	.col-md-6 .form-group {
		margin-bottom: 0rem;
	}

	.card .card-block,
	.card .card-body {
		padding: 1px 25px !important;
	}
</style>
<section class="pcoded-main-container">
	<div class="pcoded-content">

		<div class="page-header">
			<div class="page-block">
				<div class="row align-items-center">
					<div class="col-md-12">
						<div class="page-header-title">
							<h5 class="m-b-10">Daily Report</h5>
						</div>
						<ul class="breadcrumb">
							<li class="breadcrumb-item"><a href="index.php"><i class="fa fa-home"></i></a></li>
							<li class="breadcrumb-item"><a href="#!">Report</a></li>
						</ul>
					</div>
				</div>
			</div>
		</div>


		<div class="row">

			<div class="col-sm-12">
				<div class="card">
					<div class="card-header">
						<h5>Daily Report List</h5>
						<a href="add-daily-report.php"><button class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Daily Report</button></a>
					</div>
					<hr>
					<div class="row ml-3">
						<div class="col-md-3"></div>
						<div class="col-md-6">
							<div class="form-group row">
								<label for="inputEmail3" class="col-sm-4 col-form-label">Choose Task</label>
								<div class="col-sm-8">
									<select class="form-control">
										<option>Select Task</option>
										<option>Select One</option>
									</select>
								</div>
							</div>

						</div>
						<div class="col-md-3"></div>
					</div>
					<hr>
					<div class="card-body">
						<div class="dt-responsive table-responsive">
							<table id="dom-table" class="table table-striped table-bordered pre-line">
								<thead>
									<tr>
										<th rowspan="2">Sno</th>
										<th colspan="2">Time Duration</th>
										<th rowspan="2">Task</th>
										<th rowspan="2">Total Time</th>
										<th rowspan="2">Attachment</th>
										<th rowspan="2"></th>
									</tr>
									<tr>
										<th>Time In</th>
										<th>Time Out</th>
									</tr>
								</thead>
								<tbody>
									<tr>
										<td>1</td>
										<td>9:30am</td>
										<td>1:30pm </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<a href="#">
													<li>galley.jpg</li>
												</a>
												<a href="#">
													<li>galley.jpg</li>
												</a>
												<a href="#">
													<li>galley.pdf</li>
												</a>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
											</div>
										</td>
									</tr>
									<tr>
										<td>2</td>
										<td>3 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
											</div>
										</td>
									</tr>
									<tr>
										<td>3</td>
										<td>4 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
											</div>
										</td>
									</tr>
									<tr>
										<td>4</td>
										<td>1 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
											</div>
										</td>

									</tr>

									<tr>
										<td>5</td>
										<td>1 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
											</div>
										</td>
									</tr>
									<tr>
										<td>6</td>
										<td>1 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
											</div>
										</td>
									</tr>
									<tr>
										<td>7</td>
										<td>1 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
											</div>
										</td>
									</tr>
									<tr>
										<td>8</td>
										<td>1 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
												<a class="dropdown-item" href="#">Accept</a>
												<a class="dropdown-item" href="#">Reject</a>
											</div>
										</td>
									</tr>
									<tr>
										<td>9</td>
										<td>1 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
												<a class="dropdown-item" href="#">Accept</a>
												<a class="dropdown-item" href="#">Reject</a>
											</div>
										</td>
									</tr>
									<tr>
										<td>10</td>
										<td>1 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
												<a class="dropdown-item" href="#">Accept</a>
												<a class="dropdown-item" href="#">Reject</a>
											</div>
										</td>

									</tr>
									<tr>
										<td>11</td>
										<td>1 Hour </td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
												<a class="dropdown-item" href="#">Accept</a>
												<a class="dropdown-item" href="#">Reject</a>
											</div>
										</td>

									</tr>
									<tr>
										<td>12</td>
										<td>2 Hour</td>
										<td>1 Hour </td>
										<td>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</td>
										<td>10 Hour</td>
										<td>
											<ul>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
												<li>galley.jpg</li>
											</ul>
										</td>
										<td class="text-light-blue"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
											<div class="dropdown-menu">
												<a href="view-task.php" class="dropdown-item" href="#">View</a>
												<a class="dropdown-item" href="#">Accept</a>
												<a class="dropdown-item" href="#">Reject</a>
											</div>
										</td>
									</tr>

								</tbody>
							</table>
						</div>
					</div>
				</div>
			</div>




		</div>

	</div>
</section>

</div>